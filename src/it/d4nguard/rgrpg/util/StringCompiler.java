...
public class StringCompiler implements CharSequence, Appendable, Serializable
{
	...
	class StringCompilerReader extends Reader
	{
		...
		public int read(char b[], int off, int len)
		{
			...
			if (len == 0) { return 0; }
			if (pos >= StringCompiler.this.size()) { return -1; }
			if (pos + len > size())
			{
	...
	class StringCompilerTokenizer extends StrTokenizer
	{
		...
		protected List<String> tokenize(char[] chars, int offset, int count)
		{
			if (chars == null)
			{
				return super.tokenize(StringCompiler.this.buffer, 0,
								StringCompiler.this.size());
			}
			else
			{
	...
	public StringCompiler append(char[] chars, int startIndex, int length)
	{
		...
		if (startIndex < 0 || startIndex > chars.length) { throw new StringIndexOutOfBoundsException(
						"Invalid startIndex: " + length); }
		if (length < 0 || (startIndex + length) > chars.length) { throw new StringIndexOutOfBoundsException(
	...
	public StringCompiler append(String str, int startIndex, int length)
	{
		...
		if (length < 0 || (startIndex + length) > str.length()) { throw new StringIndexOutOfBoundsException(
	 ...
	 */
	public StringCompiler
					append(StringCompiler str, int startIndex, int length)
	{
		preliminars();
		if (str == null) { return appendNull(); }
		if (startIndex < 0 || startIndex > str.length()) { throw new StringIndexOutOfBoundsException(
	 ...
	 * @return this, to enable chaining
	 */
	public StringCompiler appendFixedWidthPadLeft(int value, int width,
					char padChar)
	{
	 ...
	 * @return this, to enable chaining
	 */
	public StringCompiler appendFixedWidthPadLeft(Object obj, int width,
					char padChar)
	{
		if (width > 0)
		{
	 ...
	 * @return this, to enable chaining
	 */
	public StringCompiler appendFixedWidthPadRight(int value, int width,
					char padChar)
	{
	 ...
	 * @return this, to enable chaining
	 */
	public StringCompiler appendFixedWidthPadRight(Object obj, int width,
					char padChar)
	{
		if (width > 0)
		{
	 ...
	 */
	public StringCompiler
					appendln(StringBuffer str, int startIndex, int length)
	{
		return append(str, startIndex, length).appendNewLine();
	}
	 ...
	 * @since 2.3
	 */
	public StringCompiler appendln(StringCompiler str, int startIndex,
					int length)
	{
		return append(str, startIndex, length).appendNewLine();
	}
	 ...
	 * @return this, to enable chaining
	 */
	public StringCompiler appendWithSeparators(Iterable<?> iterable,
					String separator)
	{
		if (iterable != null)
		{
	 ...
	 */
	public StringCompiler
					appendWithSeparators(Iterator<?> it, String separator)
	{
		if (it != null)
		{
			separator = toString(separator);
	 ...
	 */
	public StringCompiler
					appendWithSeparators(Object[] array, String separator)
	{
		if (array != null && array.length > 0)
		{
			separator = toString(separator);
	...
	{
		if (index < 0 || index >= size) { throw new StringIndexOutOfBoundsException(
						index); }
	...
	{
		if (obj instanceof StringCompiler) { return equals((StringCompiler) obj); }
		return false;
	}
	...
	{
		if (startIndex < 0) { throw new StringIndexOutOfBoundsException(
						startIndex); }
		if (endIndex < 0 || endIndex > length()) { throw new StringIndexOutOfBoundsException(
						endIndex); }
		if (startIndex > endIndex) { throw new StringIndexOutOfBoundsException(
						"end < start"); }
	...
	{
		startIndex = (startIndex < 0 ? 0 : startIndex);
		if (matcher == null || startIndex >= size) { return -1; }
		int len = size;
		char[] buf = buffer;
		for (int i = startIndex; i < len; i++)
		{
			if (matcher.isMatch(buf, i, startIndex, len) > 0) { return i; }
		}
		return -1;
	}
	...
	public StringCompiler insert(int index, String str)
	{
		...
		if (strLen > 0)
		{
			...
			System.arraycopy(buffer, index, buffer, index + strLen,
							size - index);
			size = newSize;
	...
	{
		startIndex = (startIndex >= size ? size - 1 : startIndex);
		if (str == null || startIndex < 0) { return -1; }
		int strLen = str.length();
		...
		else if (strLen == 0) { return startIndex; }
		return -1;
	}
	...
	{
		startIndex = (startIndex >= size ? size - 1 : startIndex);
		if (matcher == null || startIndex < 0) { return -1; }
		char[] buf = buffer;
	 ...
	 *             if the index is invalid
	 */
	public StringCompiler replace(int startIndex, int endIndex,
					String replaceStr)
	{
		endIndex = validateRange(startIndex, endIndex);
	 ...
	 */
	public StringCompiler replace(StrMatcher matcher, String replaceStr,
	...
	public CharSequence subSequence(int startIndex, int endIndex)
	{
		if (startIndex < 0) { throw new StringIndexOutOfBoundsException(
						startIndex); }
		if (endIndex > size) { throw new StringIndexOutOfBoundsException(
						endIndex); }
		if (startIndex > endIndex) { throw new StringIndexOutOfBoundsException(
						endIndex - startIndex); }
	 ...
	 *             if any index is invalid
	 */
	private StringCompiler replaceImpl(StrMatcher matcher, String replaceStr,
